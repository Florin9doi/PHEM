...
static void genmovemel (uae_u16 opcode)
{
	...
	// еее DOLATER this doesnt quite generate the right number of reads, I dont know what the extra is for!
#endif	// PALM_PERF
}
...
static void gen_opcode (unsigned long int opcode)
{
	...
	need_endlabel = 1;
#if PALM_PERF
	// еее╩still missing a read cycle sometimes.  Weird.  Don't know the address read from, so can't compute waits...
	// еее readcycles should be 2, setting to 0xFF to disable breaking
	if (curi->size == sz_byte)
	...
	need_endlabel = 1;
#if PALM_PERF
	// еее╩still missing a read cycle somewhere, when counter expires
	...
	need_endlabel = 1;
#if PALM_PERF
	// еее DOLATER less than 10% diff between best/worst times, this is worst
	...
	insn_n_cycles += 32;
#if PALM_PERF
	// еее DOLATER 38+2n extra cycles, where n = number of 1s in the <ea>
	...
	insn_n_cycles += 32;
#if PALM_PERF
	// еее DOLATER 38+2n extra cycles, where n = number of 01 or 10 patterns in <ea>w/0 at end. $5555 is worst case
	...
	need_endlabel = 1;
#if PALM_PERF
	// еее DOLATER handle extra work if exception happens
